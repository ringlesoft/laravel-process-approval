...
class ApprovableTraitTest extends TestCase
{
    ...
    public function testReturnFirstApprovalRequest(): void
    {
        $testModel = TestModel::readyForApproval();
    ...
    {
        $testModel = TestModel::readyForApproval();
        $nextApprovalStep = $testModel->nextApprovalStep();
        $validNextApprovers = User::whereHas('roles', fn($q) => $q->where('id', $nextApprovalStep->role_id))->get();
        ...
        $this->assertEquals($validNextApprovers->count(), $nextApprovers->count());
        $this->assertEquals($nextApprovers->pluck('id')->toArray(), $validNextApprovers->pluck('id')->toArray());
    }
    ...
    public function testCanBeApprovedBy()
    {
        ...
        $canBeApproved2 = $testModel->canBeApprovedBy($validNextApprovers->first());
        $this->assertTrue($canBeApproved);
        $this->assertTrue($canBeApproved2);
    }
